...
public class VietinController extends AbstractBaseController {
	...
	public ResponseEntity<?> getTokenIssue(@PathVariable(required = true) LinkType type,
		...
		VtbLinkBankBaseResponse response = null;
		...
		try {
			...
			response = IHTTPRequestUtil.sendTokenIssue(requestMap, httpCode);
			// Validate response from VTB
			responseEntity = iVietinService.validateResponse(response, httpCode, type.toString());
		} catch (Exception e) {
	...
	public ResponseEntity<?> verifyPin(@PathVariable(required = true) LinkType type,
		...
		try {
			...
			response = IHTTPRequestUtil.sendVerifyPin(requestMap, httpCode);
	...
	public ResponseEntity<?> registerOnlinePay(@PathVariable(required = true) LinkType type,
		...
		breakLabel : try {
			...
			response = IHTTPRequestUtil.sendRegisterOnlinePay(requestMap, httpCode);
			// Validate response from VTB
			responseEntity = iVietinService.validateResponse(response, httpCode, type.toString());
		} catch (Exception e) {
	...
	public ResponseEntity<?> tokenRevoke(@PathVariable(required = true) LinkType type,
		...
		try {
			...
			response = IHTTPRequestUtil.sendTokenRevoke(requestMap, httpCode);
	...
	public ResponseEntity<?> tokenReIssue(@PathVariable(required = true) LinkType type,
		...
		try {
			...
			response = IHTTPRequestUtil.sendTokenReIssue(requestMap, httpCode);
			// Validate response from VTB
			responseEntity = iVietinService.validateResponse(response, httpCode, type.toString());
		} catch (Exception e) {
	...
	public ResponseEntity<?> paymentByToken(LinkType type, PaymentByToken requestBody, HttpServletRequest request)
		...
		try {
			...
			response = IHTTPRequestUtil.sendPaymentByToken(requestMap, httpCode);
	...
	public ResponseEntity<?> paymentByOTP(LinkType type, PaymentByOTP requestBody, HttpServletRequest request)
		...
		try {
			...
			response = IHTTPRequestUtil.sendPaymentByOTP(requestMap, httpCode);
			// Validate response from VTB
			responseEntity = iVietinService.validateResponse(response, httpCode, type.toString());
			if (configLoader.isAlwaysTopupOTP()
	...
	public ResponseEntity<?> withdraw(@PathVariable(required = true) LinkType type,
		...
		try {
			...
			response = IHTTPRequestUtil.sendWithdraw(requestMap, httpCode);
	...
	public ResponseEntity<?> transactionInquiry(@PathVariable(required = true) LinkType type,
		...
		try {
			...
			response = IHTTPRequestUtil.sendTransactionInquiry(requestMap, httpCode);
			// Validate response from VTB
			VietinConnResponse responseEntity = iVietinService.validateResponse(response, httpCode, type.toString());
			return new ResponseEntity<>(responseEntity, HttpStatus.OK);
		} catch (Exception e) {
	...
	public ResponseEntity<?> getTokenIssuerPayment(@PathVariable(required = true) LinkType type,
		...
		try {
			...
			response = IHTTPRequestUtil.sendTokenIssuePayment(requestMap, httpCode);
			// Validate response from VTB
			responseEntity = iVietinService.validateResponse(response, httpCode, type.toString());
		} catch (Exception e) {
	...
	public ResponseEntity<?> refund(@PathVariable(required = true) LinkType type,
		...
		try {
			...
			response = IHTTPRequestUtil.sendRefund(requestMap, httpCode);
			// Validate response from VTB
			responseEntity = iVietinService.validateResponse(response, httpCode, type.toString());
		} catch (Exception e) {
